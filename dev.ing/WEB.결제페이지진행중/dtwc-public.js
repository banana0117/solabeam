...
jQuery(document).ready(function($) {
    ...
    var btn_img = $(".ch-pop-title-img img");

       // 선택 완료후 팝업 닫기
       btn_img.click(function(){
        
        $(".change-popup").css({"position":"fixed","top":"100%"});
        
    });
    $(".checkout-opt-pop").on("click",function(){
        $(".good").css({"position":"fixed","top":"0%"});
    });
    // 신청 완료
    $(".change-btn").on("click",function(){
        $(".change-section").fadeOut();
        $(".change-com").fadeIn();
	...
	$(".checkout-title").on("click",function(){
		$(".checkout-container").toggleClass("on");
	})

	// 배송지 팝업 닫기
	$(".bill-delivery-change").on("click",function(){
		$(".del-pop").addClass("on");
	})
	$(".del-pop-title-text img").on("click",function(){
		$(".del-pop").removeClass("on");
	});
	$(".js-order-detail-btn").on("click",function(){
		$(".del-pop").removeClass("on");
	});
