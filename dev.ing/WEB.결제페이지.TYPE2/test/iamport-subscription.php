...
// iamport subscription form ajax payment 파트 커스텀 입니다
// 주석달아놓은것중에 필요한 부분만 수정하면됩니다. 단순 클래스변경정도? 생각하면 될것같네요
// 이 파일은 수정 후 wp-content/plugins/iamport-for-woocommerce/iamport-subscription.php 에 덮어씌우면 됩니다.
...
class WC_Gateway_Iamport_Subscription extends WC_Payment_Gateway {
	...
	public function iamport_credit_card_form_fields($default_fields, $id) {
        ...
        //여기 아래부터 작업하면됩니다. 단순 모양잡는거라 그냥 클래스 추가정도? 생각하면 될듯
        // 가급적 다른 파일에서 모양을 미리한번 잡아보고 클래스명을 넣어붙이는 것을 추천합니다 잘못될시 결제안되거든요
		$args = array('fields_have_names'=>true);
		$iamport_fields = array(
			'card-number-field' => '<div class="banana-checkout-box"><div class="flexed">
			<input id="' . esc_attr( $id ) . '-card-number" class="input-text iamport-card-form iamport-required wc-credit-card-form-card-number left-radius longs" type="text" maxlength="20" autocomplete="off" placeholder="카드번호 **** **** **** ****" name="' . ( $args['fields_have_names'] ? $this->id . '-card-number' : '' ) . '" />',

			'card-expiry-field' => '
			<input id="' . esc_attr( $id ) . '-card-expiry" class="input-text iamport-card-form iamport-required wc-credit-card-form-card-expiry right-radius shorts" type="text" autocomplete="off" placeholder="' . esc_attr__( 'MM / YY', 'woocommerce' ) . '" name="' . ( $args['fields_have_names'] ? $this->id . '-card-expiry' : '' ) . '" />
			</div>',

			'card-birth-field' => '<div class="">
			<input id="' . esc_attr( $id ) . '-card-birth" style="width:100%;" class="input-text iamport-card-form iamport-required wc-credit-card-form-card-birth left-radius right-radius" type="password" autocomplete="off" placeholder="' . esc_attr__( '생년월일6자리 또는 사업자등록번호10자리', 'iamport-for-woocommerce' ) . '" name="' . ( $args['fields_have_names'] ? $this->id . '-card-birth' : '' ) . '" maxlength="10"/>
			</div>',

			'card-pwd-field' => '<div class="flexed">
			<input id="' . esc_attr( $id ) . '-card-pwd" class="input-text iamport-card-form iamport-required wc-credit-card-form-card-pwd left-radius longs" type="password" autocomplete="off" placeholder="' . esc_attr__( '카드비밀번호 앞2자리', 'iamport-for-woocommerce' ) . '" name="' . ( $args['fields_have_names'] ? $this->id . '-card-pwd' : '' ) . '" maxlength="2"/>
			',
		);
        ...
        // 5만원이상 할부 조건 삭제
        // 클래스명 추가해서 붙이면 될것같네요
            $iamport_fields['card-quota-field'] = '
			<select id="' . esc_attr( $id ) . '-card-quota" class="input-text wc-credit-card-form-card-quota right-radius shorts" autocomplete="off" name="' . ( $args['fields_have_names'] ? $this->id . '-card-quota' : '' ) . '">
