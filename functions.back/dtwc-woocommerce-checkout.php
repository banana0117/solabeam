 ...
 * 
 * 이 파일은 
 * www/wp-content/plugins/delivery-times-for-woocommerce/admin 
 * 경로에 덮어 씌우시기 바랍니다. 감사합니다.
 * 
 */
...
function dtwc_delivery_date_checkout_field_process() {
    ...
    $message = __( '첫 배송 출발일을 선택해주세요.', 'dtwc' );
...
//YJcustom
	if ( isset( $_POST ['dtwc_delivery_date'] ) && '' != $_POST ['dtwc_delivery_date'] ) {
	$delidate = $_POST['dtwc_delivery_date'];
	$yoil = array("일","월","화","수","목","금","토");
	$deliday = $yoil[date('w', strtotime($delidate))];
		add_post_meta( $order_id, 'deliday',  sanitize_text_field( $deliday ) );
	}
